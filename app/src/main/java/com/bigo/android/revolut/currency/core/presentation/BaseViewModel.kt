...
typealias ActionToResultMapper<A, R> = suspend FlowCollector<R>.(A) -> Unit
...
abstract class BaseViewModel<V : ViewState, I : Intent> : ViewModel() {
    ...
    ): Flow<R> = transform(transformer).buffer().flowOn(Dispatchers.Default)

    protected inline fun <reified A : Action, reified R : Result> Flow<A>.actionToResultWithChannel(
        noinline transformer: suspend ProducerScope<R>.(A) -> Unit
    ): Flow<R> = channelFlow {
        collect {
